...
namespace GrooveSharedUtils
{
	public abstract class BaseEquipmentMasterBehavior : MonoBehaviour
	{
		...
		internal static void Init()
		{
			...
			foreach (EquipmentDefAssociationAttribute itemDefAssociationAttribute in attributeList)
			{
				...
				else
				{
					...
					else
					{
						...
						{
							GroovyLogger.Log(LogLevel.Error, string.Format("{0}.{1} returned an EquipmentDef that's not registered in the EquipmentCatalog. result={2}", 
								methodInfo.DeclaringType.FullName, 
								methodInfo.Name,
								equipmentDef));
						}
			...
			if (BaseEquipmentMasterBehavior.shared.itemTypePairs.Length <= 0)
			{
				return;
			}
